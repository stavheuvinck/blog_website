...
export const databaseBlogPostsId = process.env.NOTION_BLOG_POSTSDATABASE_ID;
    ...
    <div>
      ...
      <main className={styles.container}>
        <header className={styles.header}>
          ...
          <h1>Hi there, I am Stav Heuvinck!</h1>
          <p>
          I am studying applied computer science at Howest. Currently I am working on my internship and bachelor thesis. Last year I chose the AI business professional track because I have been very interested in the rapidly evolving technology of AI for several years. The possibilities around it are just so diverse. So I don't regret my choice for a second. Besides AI, I am also someone who likes to build visual projects in my spare time. These are mostly native apps and websites. For example, I'm currently writing an application for my dad's company. As a window cleaner/graphic designer, he has to do a lot of planning, messaging, sending out offers,... and I want to combine and automate all this in 1 app to reduce the daily stress. So this is currently my work and passion, for more information about my career you can always check my curriculum vitae. Hope you find my projects interesting and maybe moves you to try out something similar too! Have fun reading!
          </p>
          <p>🧑‍💻 Intern at <b className={styles.back}>Robovision.AI</b></p>
          <p>👶 Age: 22</p>
          <p>🏘️ Denderbelle, East-Flanders Belgium</p>
          <Link href="cv">
                  <a>📄 Curriculum vitae</a>
          </Link>
        </header>

        <h2 className={styles.heading}>Blog ✍️</h2>
        <ol className={styles.posts}>
          {posts.map((post) => {
            const date = new Date(post.properties.Date.rich_text).toLocaleString(
              "en-US",
              {
                month: "short",
                day: "2-digit",
            ...
            return (
              <li key={post.id} className={styles.post}>
                ...
                <p className={styles.postDescription}><Text text={post.properties.Date.rich_text} /></p>
        ...
        <ol className={styles.posts}>
          {works.map((work) => {
            const date = new Date(work.last_edited_time).toLocaleString(
              "en-US",
              {
                month: "short",
                day: "2-digit",
            ...
            return (
              <li key={work.id} className={styles.post}>
                <h3 className={styles.postTitle}>
                  <Link href={`/${work.id}`}>
                    <a>
                      <Text text={work.properties.Name.title} />
                    </a>
                  </Link>
                </h3>
...
export const getStaticProps = async () => {
  const database_blog = await getDatabase(databaseBlogPostsId);
  const database_work = await getDatabase(databasePreviousWorkId);
